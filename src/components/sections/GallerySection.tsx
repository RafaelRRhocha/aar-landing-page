...
export function GallerySection() {
  ...
  return (
    <section id="galeria" className="py-16 sm:py-20 desktop:py-32 bg-cream overflow-hidden">
      <Container>
        <AnimatedSection>
          <SectionTitle
            title="Galeria"
              ...
              <button
                ...
                className={`relative px-5 py-2.5 text-sm font-medium transition-all duration-300 rounded-lg ${
                  activeCategory === category.id
                    ? "text-white"
                    : "text-foreground/70 hover:text-foreground bg-white border border-cream-dark hover:border-primary/30"
                }`}
              >
                {activeCategory === category.id && (
                  <motion.div
            ...
            >
              {currentCategory && (
                <div className="text-center mb-8">
                  <p className="text-foreground/60 text-sm sm:text-base">{currentCategory.description}</p>
                </div>
              )}
